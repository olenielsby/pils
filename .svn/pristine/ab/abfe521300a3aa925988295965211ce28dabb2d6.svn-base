...
namespace PILS
{
	Writing::Writing(Runner &run, const Language *language, const Any *thing)
		: run(run), language(language), spacing(tight), labelCount(0), level(0), flatLevel(0), lastName(NULL)
	{
		...
		if(thing->isMultipleReferenced())
		{
			...
			if (find != labels.end() && find->second)
			{
				...
				if (find->second == -1)
				{
					...
					spacing = tight;
					return thing->write(*this, WRITING_ARGUMENT, level, NULL);
				}
				else
				{
	...
	bool PilsString::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		writing.lastName = NULL; // avoid confusing name repeaters
	...
	bool Cliche::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		if (--level)
		{
			...
			writing.write(head, WRITING_CONSTANT, level, NULL);
			writing.level--;
			...
			for (size_t i = 0; i < count; i++)
			{
				...
				if (attributes[i] != &Empty::singleton)
				{
					writing.optionalSpace();
					writing.level++;
					writing.write(attributes[i], WRITING_CONSTANT, level, NULL);
					writing.level--;
					writing.optionalSpaceOrBreak();
				}
			}
	...
	void Language::Index::Record::write(Writing &writing, bool stringwise) const
	{
		...
		if (stringwise)
			local->write(writing, WRITING_CONSTANT, 0, NULL);
		else
		{
	...
	bool NodeConstant::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		writing.write(head, WRITING_NAME, level, NULL);
		writing.write(':');
		writing.spacing = Writing::space;
		...
		for (size_t i = cliche->count - 1; i + 1; i--)
		{
			...
			else
			{
				...
				if (name == &Empty::singleton)
				{
					...
					{
						writing.flatLevel--;
						writing.write(v, WRITING_CONSTANT, level, NULL);
						writing.flatLevel++;
					}
				}
				else
				{
					...
					writing.write('.');
					writing.spacing	= Writing::tight;
					writing.write(cliche->attributes[i], WRITING_NAME, level, NULL);
	...
	bool ListConstant::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		if (shortlist && state != WRITING_CONSTANT_ELEMENT)
		{
			for (size_t i = 0; i < c; i++)
			{
				const Integer *ec;
				const Any *const *ee;
				if ((element[i]->isList(ee, ec) && ec->value != 1) || element[i]->as_String() != NULL)
				{
					shortlist = false;
					break;
				}
		...
		if (shortlist)
		{
			for (size_t i = 0; i < c; i++)
			{
				...
				writing.write(e, WRITING_CONSTANT_SHORT_ELEMENT, level, NULL);
			}
		}
		else
		{
			for (size_t i = 0; i < c; i++)
			{
				...
				{
					writing.write(e, WRITING_CONSTANT_ELEMENT, level, NULL);
				}
	...
	bool NodeExpress::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		else
		{
			...
			if (const Any* v = getAttribute(*head))
			{
				writing.write('.');
				const Slot *slot = v->as_Slot();
				if (slot == NULL||slot->element[0] != head)
				{
		...
		for (size_t i = cliche->count - 1; i + 1; i--)
		{
			...
			else
			{
				...
				else
				{
					...
					writing.write('.');
					writing.spacing = Writing::tight;
					writing.write(name, WRITING_NAME, level, NULL);
				}
				...
				else if (v == &Empty::singleton && !writing.language->withNoSugar()) v = NULL;
				if (v)
				{
	...
	bool Escape::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		writing.write(element[0], WRITING_EXPRESSION, level, NULL);
		if (wrap)
		{
			writing.level--;
			writing.optionalSpace();
			...
			&& (skin = writing.language->nameLookup(head, record)) != NAMESKIN_NONE)
		{
			...
			if (node)
			{
				if (cliche->count == 1 && cliche->attributes[0] == &Empty::singleton)
					writing.write(node->element[0], WRITING_ARGUMENT, level, NULL);
				else
				{
					writing.writeSpaceOrBreak();
					writing.write('[');
			...
			return node == NULL && wrap == false;
		}

		return PokerShort::write(writing, state, level, NULL);
	}
	...
	bool Call::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		else return PokerShort::write(writing, state, level, NULL);
	}
	...
	bool CallWho::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		const Any *const *callElement = NULL;
		const Cliche *callCliche = NULL;
		const ClicheShort *name = NULL;
		Language::Index::Record record;
		...
		if (prefix)
		{
			...
			writing.spacing = Writing::space;
			mustDot = writing.write(element[1], WRITING_OPERAND, level, NULL);
			writing.level--;
		}
		else
		{
			...
			case NAMESKIN_JOKER:
				...
				break;
			case NAMESKIN_OPERATOR:
				mustDot = writing.write(callElement[0], WRITING_OPERAND, level, NULL);
				break;
			case NAMESKIN_ADDITION:
				mustDot = writing.write(callElement[0], WRITING_ADDEND, level, NULL);
				break;
			case NAMESKIN_RELATION:
				mustDot = writing.write(callElement[0], WRITING_COMPARAND, level, NULL);
				break;
			case NAMESKIN_NAME:
			case NAMESKIN_ASTERISKED:
			case NAMESKIN_STRING:
				if (callElement)
				{
					...
					if (callCliche->count == 1 && callCliche->attributes[0] == &Empty::singleton)
						mustDot = writing.write(callElement[0], WRITING_ARGUMENT, level, NULL);
						...
						for (size_t i = callCliche->count - 1; i + 1; i--)
						{
							...
							else
							{
								...
								else
								{
									...
									writing.write('.');
									writing.spacing = Writing::tight;
									writing.write(callCliche->attributes[i], WRITING_NAME, level, NULL);
								}
			...
			default:
				...
				{
					mustDot = writing.write(element[0], WRITING_EXPRESSION, level, (mustDot ? &Empty::singleton : NULL));
				}
				else
				{
					if (mustDot)
					{
						...
						writing.write('.');
						writing.spacing = Writing::dot;
						mustDot = writing.write(element[0], WRITING_DOT_ARGUMENT, level, NULL);
					}
					else mustDot = writing.write(element[0], WRITING_ARGUMENT, level, NULL);
				}
			}
			writing.flatLevel--;
		}
...
//				if (absValue < (double)mantissa / scale) mantissa--;
	...
	bool Listener::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		if (dot == &Empty::singleton)
		{
			writing.spaceDot();
		...
		writing.write('?');
		writing.spacing = Writing::tight;
		writing.write(channel, WRITING_NAME, level, NULL);
		writing.write('}');
		writing.spacing = Writing::space;
		return false;
	}
	...
	bool Ruleset::write(Writing &writing, WriteState state, long level, const Constant *dot) const
	{
		...
		for (size_t i = 0; i < (size_t)rulelist.count->value; i++)
		{
			...
			{
				writing.optionalSpace();
				writing.write(match, WRITING_EXPRESSION, level, NULL);
			}
			writing.level--;
			writing.optionalSpaceOrBreak();
			writing.write('|');
			...
			if (action != &Empty::singleton)
				writing.write(action, WRITING_EXPRESSION, level, NULL);
			writing.level--;
			writing.optionalSpaceOrBreak();
			writing.flatLevel--;
			writing.write('}');
	...
	const Any *NodeExpress::labeling(Writing &writing) const
	{
		if (writing.labeling(this))
		{
			...
			for (long i = cliche->count - 1; i >= 0; i--)
			{
				...
				if(n == NULL || n->cliche != &BuiltinClicheCall::singleton || n->element[0] != e)
				{
					while(e) e=e->labeling(writing);
					e = element[i];
				}
