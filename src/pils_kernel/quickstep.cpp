...
namespace PILS
{
	...
	{
        if (const Any *when = value.getWhen())
        {
            when->unduplicateReference();
            return when;
        }
	...
	void QuickActionOk::action(Runner &run) const
	{
		...
		*stack = run.calling.closure->element[0];
		...
		for(;stack < (const Any**)run.sink;stack++) (*stack)->retain();
		thing.retain();
	...
	const QuickCalling *QuickBuildList::build(Runner &run, void **stack) const
	{
		...
		for (size_t i = 0; i < count && constant; i++)
			constant = (*((const Any **)(stack)))->as_Constant() != nullptr;
		const Any *value;
		if (constant)
	...
	{
		return matching(run, stack, value, nullptr);
	}
	...
	const QuickCalling *QuickFetchSpecialType::match(Runner &run, void **stack, const NodeExpressLong &value) const
	{
		...
		public:
			...
			{
				return (type = ((Special &)special).type()) != nullptr;
			}
			const Constant *type;
		} typeExtractor;
