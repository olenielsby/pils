...
namespace PILS
{
			...
			case ';':
				if (at + 1 < end)
				{
					...
					case ':':
						...
						parsing.at++;
						parsing.at++;
                        return ((Parse *)run.sink)->nodehead(run, parsing, Empty::get());
					case '(':
					case ')':
					case 0x00:
					case 0x01:
	...
	const PilsString *Parsing::string()
	{
		...
		if (!counting.pass(at, end)) return nullptr;
	...
	const Constant *Parsing::date(Runner &run)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			if (at == end || *at < '0' || *at > '9') return nullptr;
			split.month *= 10;
			split.month += *at++ - '0';
		}
		if (at == end || *at++ != '-') return nullptr;
		for (int i = 0; i < 2; i++)
		{
			if (at == end || *at < '0' || *at > '9') return nullptr;
			split.day *= 10;
		...
		if (split.day < 1 || split.day > maxday) return nullptr;
		if (at != end && *at == 'T')
		{
			at++;
			for (int i = 0; i < 2; i++)
			...
			if (split.hour > 23) return nullptr;
			if (at != end && *at == ':')
			{
				at++;
				for (int i = 0; i < 2; i++)
				...
				if (split.minute >= 60) return nullptr;
				if (at != end && *at == ':')
				{
					at++;
					for (int i = 0; i < 2; i++)
					...
					if (split.second > 60) return nullptr;
					if (at != end && *at == '.')
					{
						at++;
						for (int i = 0; i < 3; i++)
	...
	const Step *Parsing::useLabel(Runner &run, int index)
	{
		...
		if (const Any *value = find == labels.end() ? nullptr : find->second)
		{
			value->retain();
	...
	{
		if (labels.insert(std::pair<int, const Any *>(index, static_cast<const Any *>(nullptr))).second)
		{
	...
	{
        closePhrase(run, parsing, PARSE_PHRASE, Empty::get());
	...
	void Parse::close(Runner &run, Parsing &parsing, ParseLevel level)
	{
		...
		{
            close(run, parsing, level, (const Any *)Empty::get());
		}
	}
...
//		close(run, parsing, level);
	...
	{
		at = parsing.at;
        return attributeName(run, parsing, Empty::get());
	}
	...
	{
        return constant(run, parsing, Empty::get());
	}
	...
	void ParseConstant::close(Runner &run, Parsing &parsing, ParseLevel level)
	{
		if (level == PARSE_ELEMENT || level > PARSE_ATTRIBUTE)
		{
            close(run, parsing, level, Empty::get());
		}
	}
	...
	void ParseSquare::close(Runner &run, Parsing &parsing, ParseLevel level)
	{
		...
		{
			run.sink = (Sink *)(this + 1);
            new (run.allocate(sizeof(ParseEndSquare))) ParseEndSquare(parsing.at, Empty::get());
		}
