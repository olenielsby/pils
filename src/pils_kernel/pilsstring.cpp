...
namespace PILS
{
	...
	const NodeConstantShort *BuiltinClicheRange::newNode(const Constant *&link, const Integer *value) const
	{
		if (value->value >= 0)
	...
	const Step *PipePlusSlackRange::pipeBegin(Runner &run)
	{
		...
		if (!range)
		{
			...
			run.sink = &next;
			new (run.allocate(sizeof(PipesourceListRange))) PipesourceListRange(nullptr, (size_t)0);
		}
	...
	const NodeConstantShort *BuiltinClicheSearchAbstract::newNode(const Constant *&link, const PilsString *value) const
	{
        return new const SearchString(link, *this, value);
	}

	const NodeConstantShort *BuiltinClicheSearchAbstract::newNode(const Constant *&link, const ListConstant *value) const
	{
	...
	const Step *PipesourceSplit::step_(Runner &run) const
	{
		...
		if (at)
		{
			...
			if (!item)
			{
				...
				((PipesourceSplit *)this)->at = nullptr;
			}
	...
	{
		const Any *const *element = nullptr;
		const Integer *count;
		if (value->isList(element, count) && ((size_t)count->value & (size_t)1) == 0)
		{
			for (const Any *const *const end = element + count->value; element != nullptr && element < end;)
			{
				const PilsString *search = (*element++)->as_String();
				const PilsString *replace = (*element++)->as_String();
				if (search == nullptr || invalidSearchstringLength(search->count->value) || replace == nullptr)
					element = nullptr;
			}
		}
		else element = nullptr; // odd lengt = misfit;
		if (element)
	...
	{
		const PilsString *result = nullptr;
	...
	const Step *PipeCharacterEncoder::pipeEnd(Runner &run)
	{
		...
		const PilsString *string = PilsString::get(count?&this->string[0]:nullptr, count);
