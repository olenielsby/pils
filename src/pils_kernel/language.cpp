...
namespace PILS
{
		...
		if (call.cliche->count == 1 && call.cliche->attributes[0] == &Empty::singleton)
		{
			...
			else if (operation == &Builtin::name.write)
			{
				...
				const PilsString *result;
				{
					...
					call.element[0]->write(writing, WRITING_EXPRESSION, -1L, nullptr);
					writing.done();
			...
			else if (operation == &Builtin::name.name_)
			{
				...
				if (name)
				{
					Index::Record record(nullptr, nullptr);
			...
			if (operation == &Builtin::name.write)
			{
				...
				long level = -1;
				{
					bool overflow;
					{
						...
						call.element[0]->write(writing, WRITING_EXPRESSION, level, nullptr);
						writing.done();
	...
	const NodeConstantShort *BuiltinClicheLanguage::newNode(const Constant *&link, const NodeConstant *value) const
	{
		...
		{
			//Get default names
			const NodeConstant *defaultNames = nullptr;
			for (size_t namespaceIndex = 1;; namespaceIndex++)
			{
				...
				if (names == nullptr) goto invalidLanguage;
				defaultNames = names;
				break;
			}
			...
			for (size_t namespaceIndex = 1; namespaceIndex < value->cliche->count; namespaceIndex++)
			{
				...
				if (prefix == nullptr || names == nullptr) goto invalidLanguage;
			...
			if (defaultNames == nullptr) goto invalidLanguage;
			for (size_t i = 0; i < defaultNames->cliche->count; i++)
			{
				...
				index->translation[value] = Language::Index::Record((const PilsString*)nullptr, local);
			}

			// index the null namespace prefix if open
			if (defaultUrl)
				index->prefix[defaultUrl] = nullptr;
	...
	NameSkin Language::nameLookup(const ClicheShort *name, Index::Record &record) const
	{
		...
		else
		{
			...
			if (local == nullptr) return NAMESKIN_NONE; // Not a name
