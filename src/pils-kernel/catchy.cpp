...
namespace PILS
{
	...
	const Step *ImplicitParameterShort::catching(Runner &run, Catch &catching) const
	{
		...
		Empty::singleton.addReference();
		const Constant *bindingAttribute[2] = {&Empty::singleton, name};
		const Cliche *bindingCliche = Empty::singleton.clichefy(bindingAttribute, 2);
		...
		value[0]->addReference();
		value[1]->addReference();
	...
	const Step *SinkTryCallingWho::called(Runner &run, const Constant &call)
	{
		...
		run.sink = new ((void*)((SinkTryCalling*)(this + 1) - 1)) SinkTryCalling(where_, callWho);
