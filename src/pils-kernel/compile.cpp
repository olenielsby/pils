...
namespace PILS
{
	...
	{
		return (cliche->head != &Empty::singleton)?(const char *)cliche + 1:NULL;
	}
	...
	void TypecheckListen::compileTypecheck(Compiling &compiling, const CallWho &pattern) const
	{
		compiling.typecheckListener();
		pattern.whoAttribute()->compilePattern(compiling);
	}
	...
	void ImplicitParameterTrailer::compileAction(Compiling &compiling) const
	{
		...
		tag->addReference();
		cliche->attributes[1]->addReference();
	...
	void CompilingPrescan::beginRule(const void *ruleIndex)
	{
		...
		names.insert(names.begin(), &Empty::singleton);
	...
	void CompilingPrescan::endRule(const void *ruleIndex)
	{
		...
		clicheHeadEmptyIfBuildable->addReference();
		if (count == 1)
	...
	void CompilingGenerate::endIndex(bool more)
	{
		if (indexing)
		{
			...
			code = putKey + count + 1;
			const QuickCalling *nextRule = (const QuickCalling*)(more ? code : NULL);
	...
	void CompilingPrescan::typecheckListener()
	{
		size += sizeof(QuickTestListener);
	}

	void CompilingGenerate::typecheckListener()
	{
		new (allocate(sizeof(QuickTestListener))) QuickTestListener(currentFailChain);
	}
	...
	void CompilingGenerate::done()
	{
		...
		else resolveChain(unindexedFailChain, NULL);
		resolveChain(unindexedAssignmentFailChain, NULL);
	}
