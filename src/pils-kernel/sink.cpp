...
namespace PILS
{
	...
	const Step *Runner::callingStackOverflow()
	{
		...
		Builtin::name.stackOverflow.addReference();
	...
	PassingMindMap::~PassingMindMap()
	{
		for (std::map<const Constant*, const Any*>::iterator i = mind.begin(); i != mind.end(); i++)
		{
	...
	const Any *PassingMindMap::mindGet(const Constant &key) const
	{
		std::map<const Constant*, const Any*>::const_iterator found = mind.find(&key);
		if (found != mind.end())
		{
	...
	const Any *PassingMindMap::mindSet(const Constant *key, const Any *value) const
	{
		std::pair<std::map<const Constant*, const Any*>::iterator, bool> insert = mind.insert(std::pair<const Constant*, const Any*>(key, value));
		if (insert.second)
			return NULL;
		else
		{
	...
	void PassingMindMap::mindRelease() const
	{}
}
