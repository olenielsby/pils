...
//		run.sink = &sink;
	...
	Sink *SinkRecall::kick(Runner &run)
	{
		...
		for (std::map<const Constant*, const Any*>::iterator i=map.begin(); i != map.end(); i++)
		{
			...
			if(i->second) i->second->releaseReference();
		}
		map.~map();
	...
	const Step *Recalling::calling(Runner &run, const Constant &call) const
	{
		...
		if (tagged)
		{
			...
			std::pair<std::map<const Constant*, const Any*>::iterator, bool> insert = sinkRecall.map.insert(std::pair<const Constant *, const Any *>(&call, static_cast<const Any *>(NULL)));
			if (insert.second)
			{
				call.addReference();
